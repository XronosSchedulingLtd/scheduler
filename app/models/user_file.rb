...
class UserFile < ActiveRecord::Base
  ...
  #
  #  The business of encoding is quite a tricky one.  With uploaded
  #  files we generally take an attitude of being completely dumb.
  #  What we are given we save, and we don't worry about the bytes
  #  which the file contains.
  #
  #  However, if a file has been generated locally then it may already
  #  be open and an encoding may have been specified - especially if it's
  #  a spreadsheet generated by the Spreadsheet gem.
  #
  #  In that case, we need to use the same encoding when copying the
  #  already open file.
  #
  #  So - for most files this parameter is not specified, but for locally
  #  generated ones it may be.
  #
  ...
  #
  #  Replace an existing user file's contents.
  #
  def replace_contents(file_info)
    File.open(file_full_path_name,
              "wb#{@encoding ? ":#{@encoding}" : ""}") do |file|
      file.write(file_info.read)
    end
    self.file_size = file_info.size
    #
    #  If the file size happens to be exactly as it was before
    #  then a save call will be ignored.
    #
    if self.changed?
      self.save
    else
