      ...
      ec.repetition_start_date.upto(ec.repetition_end_date) do |date|
        if ec.happens_on?(date, week_identifier.week_letter(date), todays_date)
          ...
          commitments.each do |commitment|
            ...
            #
            puts "Considering commitment to #{commitment.event.body}"
            if commitment.event == event
              puts "This is our original event"
            end
            if commitment.event.in_collection?(ec)
              puts "This event is in our own collection"
            end
